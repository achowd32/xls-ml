...
// TODO: https://github.com/google/xls/issues/4125 - This is brittle but
// preserves the pre-existing behavior of explicit state access. We need to
// replace string keys with `StateElement` keys.
std::string GetStateElementName(const Expr* expr) {
  if (expr->kind() == AstNodeKind::kAttr) {
    return std::string(absl::down_cast<const Attr*>(expr)->attr());
  }
    ...
    if (const ConstFor* loop = dynamic_cast<const ConstFor*>(node)) {
      ...
      if (unrolled_for.has_value()) {
        ...
        return absl::FailedPreconditionError(absl::StrCat(
            "unroll_for! or const for should have been unrolled by now at: ",
...
absl::StatusOr<BValue> FunctionConverter::Use(const AstNode* node) const {
  ...
  XLS_RET_CHECK(std::holds_alternative<CValue>(ir_value))
      << "Expected CValue, got " << ir_value.index() << " for "
      << node->ToString();
...
absl::Status FunctionConverter::HandleLet(const Let* node) {
  ...
  // The let node itself gets a dummy BValue for a channel assignment, which
  // will not be used in practice, but this function always maps a meaningful
  // IrValue to each NameDef on the LHS.
...
void FunctionConverter::BuildMapIterFunction(FunctionBuilder* syn,
                                             const SourceInfo& loc,
  ...
  if (self_value.has_value()) {
    self_in_syn = syn->Param("context_struct", (*self_value).GetType());
  }
  BValue input_array = syn->Param("input_array", arg.GetType());
  BValue arr_idx = syn->ArrayIndex(input_array, {loop_idx});
  std::vector<BValue> args =
      (!self_value.has_value() ? std::vector<BValue>({arr_idx})
                               : std::vector<BValue>({self_in_syn, arr_idx}));
  BValue inv = syn->Invoke(
      args, f, loc, absl::StrFormat("map_%s_call", mapped_fn_identifier));
  syn->ArrayUpdate(array, inv, {loop_idx}, loc, "map_iteration");
}

void FunctionConverter::BuildMapIterFunctionWithToken(
    FunctionBuilder* syn, const SourceInfo& loc, xls::Type* output_type,
    std::optional<BValue> self_value, BValue arg, xls::Function* f,
    std::string mapped_fn_identifier) {
  BValue loop_idx = syn->Param("idx", package()->GetBitsType(32), loc);
  BValue loop_data = syn->Param(
      "token_and_array",
      package()->GetTupleType({package()->GetTokenType(), output_type}), loc);
  BValue activated = syn->Param("activated", package()->GetBitsType(1), loc);
  BValue self_in_syn;
  if (self_value.has_value()) {
    self_in_syn = syn->Param("context_struct", (*self_value).GetType());
  }
  BValue input_array = syn->Param("input_array", arg.GetType());
  BValue array_val = syn->TupleIndex(loop_data, 1, loc, "out_array_value");
  BValue token_idx = syn->TupleIndex(loop_data, 0, loc, "token_value");
  BValue arr_idx = syn->ArrayIndex(input_array, {loop_idx}, loc, "input_value");
  std::vector<BValue> args =
      (!self_value.has_value()
           ? std::vector<BValue>({token_idx, activated, arr_idx})
           : std::vector<BValue>({token_idx, activated, self_in_syn, arr_idx}));
  BValue tok_and_new_val = syn->Invoke(
      args, f, loc, absl::StrFormat("map_%s_call", mapped_fn_identifier));
  syn->Tuple({syn->TupleIndex(tok_and_new_val, 0),
...
absl::StatusOr<BValue> FunctionConverter::HandleMap(const Invocation* node) {
  ...
  if (auto* callee_ref = dynamic_cast<FunctionRef*>(fn_node);
    ...
    if (IsBuiltinParametricNameRef(name_ref)) {
      VLOG(5) << "Map of parametric builtin: " << name_ref->identifier();
      return DefMapWithBuiltin(node, name_ref, node->args()[0],
                               *node_parametric_env.value());
    }
    XLS_ASSIGN_OR_RETURN(
        mapped_fn, module_->GetMemberOrError<Function>(name_ref->identifier()));
    ...
    XLS_ASSIGN_OR_RETURN(mapped_fn, (*info)->module->GetMemberOrError<Function>(
                                        colon_ref->attr()));
  } else {
    return absl::UnimplementedError(
  ...
  std::optional<BValue> self_value = std::nullopt;
  std::vector<BValue> args({arg});
  if (self != nullptr) {
    XLS_ASSIGN_OR_RETURN(self_value, Use(self));
    args = {*self_value, arg};
  }
  if (is_implicit_token) {
    ...
    XLS_RET_CHECK(implicit_token_data_.has_value()) << absl::StreamFormat(
        ...
        mapped_fn->identifier(), node->span().ToString(file_table()));
    ...
    std::vector<BValue> args_with_control({control_pred});
    for (const BValue& arg : args) {
      args_with_control.push_back(arg);
    }
    return DefWithStatus(
        node, [&](const SourceInfo& loc) -> absl::StatusOr<BValue> {
          FunctionBuilder syn(synthetic_function_name, package());
          BuildMapIterFunctionWithToken(&syn, loc, output_type, self_value, arg,
                                        f, mapped_fn->identifier());
          XLS_ASSIGN_OR_RETURN(
          ...
          BValue arg_and_tok = function_builder_->CountedFor(
              ...
              arg_size, /*stride=*/1, loop_fn, args_with_control, loc,
              absl::StrFormat("map_invocation__%s", mapped_fn->identifier()));
  ...
  return DefWithStatus(
      node, [&](const SourceInfo& loc) -> absl::StatusOr<BValue> {
        FunctionBuilder syn(synthetic_function_name, package());
        BuildMapIterFunction(&syn, loc, output_type, self_value, arg, f,
        ...
        return function_builder_->CountedFor(
            function_builder_->Literal(ZeroOfType(output_type)),
            /*trip_count=*/arg_size, /*stride=*/1, loop_fn, args, loc,
            absl::StrFormat("map_invocation__%s", mapped_fn->identifier()));
      });
}
...
absl::Status FunctionConverter::HandleBuiltinRead(const Invocation* node) {
  ...
  BValue new_read =
      builder_ptr->StateRead(BStateElement(state_element, builder_ptr), active,
                             node->label(), ToSourceInfo(node->span()));
...
absl::Status FunctionConverter::HandleBuiltinWrite(const Invocation* node) {
  ...
  BValue active = implicit_token_data_->create_control_predicate();
  std::string state_name = GetStateElementName(target);
  BValue state_read_called = state_read_called_by_state_name_[state_name];
  ...
  if (std::holds_alternative<xls::StateElement*>(*ir_val)) {
    ...
    builder_ptr->Next(BStateElement(state_element, builder_ptr), update_val,
                      active, node->label());
  } else {
  ...
  return absl::visit(
      Visitor{
          ...
          [](ChannelInterface* ci) -> absl::StatusOr<ChanRef> {
            ...
            return absl::InvalidArgumentError(
                absl::Substitute("Unexpected ChannelInterface in IrValue: `$0`",
                                 ci->ToString()));
          },
...
absl::StatusOr<SendChannelRef> FunctionConverter::IrValueToSendChannelRef(
    const IrValue& ir_value) {
  XLS_ASSIGN_OR_RETURN(SendChannelRef channel_ref,
                       (IrValueToChannelRef<SendChannelEnd, SendChannelRef,
                                            SendChannelInterface>(ir_value)));
  if (options_.lower_to_proc_scoped_channels &&
      std::holds_alternative<Channel*>(channel_ref)) {
    // Go back from the channel to the channel interface
    XLS_ASSIGN_OR_RETURN(ChannelInterface * ci,
                         ChannelToInterface(std::get<Channel*>(channel_ref),
                                            ChannelDirection::kOut));
    return dynamic_cast<SendChannelInterface*>(ci);
  }
  return channel_ref;
}

absl::StatusOr<ReceiveChannelRef> FunctionConverter::IrValueToReceiveChannelRef(
    const IrValue& ir_value) {
  XLS_ASSIGN_OR_RETURN(
      ReceiveChannelRef channel_ref,
      (IrValueToChannelRef<RecvChannelEnd, ReceiveChannelRef,
  ...
  if (options_.lower_to_proc_scoped_channels &&
    ...
    return dynamic_cast<ReceiveChannelInterface*>(ci);
  }
  return channel_ref;
}

absl::StatusOr<ChannelInterface*> FunctionConverter::ChannelToInterface(
    Channel* channel, ChannelDirection direction) {
  XLS_RET_CHECK(options_.lower_to_proc_scoped_channels)
      << "Should not call ChannelToInterface if not lowering to proc-scoped "
         "channels";
  XLS_RET_CHECK_NE(channel, nullptr) << "channel cannot be null";
  ProcBuilder* builder_ptr =
      dynamic_cast<ProcBuilder*>(function_builder_.get());
  ...
  if (direction == ChannelDirection::kIn) {
    BReceiveChannel rcv =
        builder_ptr->GetReceiveChannelInterface(channel->name());
    XLS_RETURN_IF_ERROR(builder_ptr->GetError());
    return rcv.channel_interface();
  }
  BSendChannel snd = builder_ptr->GetSendChannelInterface(channel->name());
  XLS_RETURN_IF_ERROR(builder_ptr->GetError());
  return snd.channel_interface();
}

absl::StatusOr<ChannelInterface*> FunctionConverter::IrValueToChannelInterface(
  ...
  if (direction == ChannelDirection::kIn) {
    ...
    XLS_RET_CHECK(std::holds_alternative<ReceiveChannelInterface*>(ref));
    return std::get<ReceiveChannelInterface*>(ref);
  }
  XLS_ASSIGN_OR_RETURN(SendChannelRef ref, IrValueToSendChannelRef(ir_value));
  XLS_RET_CHECK(std::holds_alternative<SendChannelInterface*>(ref));
...
absl::Status FunctionConverter::HandleBuiltinSend(const Invocation* node) {
  ...
  if (implicit_token_data_.has_value()) {
    ...
    result = builder_ptr->SendIf(
        BSendChannelRef(channel_ref), token_value,
        implicit_token_data_->create_control_predicate(), payload_value);
  } else {
    result = builder_ptr->Send(BSendChannelRef(channel_ref), token_value,
                               payload_value);
  }
...
absl::Status FunctionConverter::HandleBuiltinSendIf(const Invocation* node) {
  ...
  if (implicit_token_data_.has_value()) {
    ...
    result = builder_ptr->SendIf(BSendChannelRef(channel_ref), token_value,
                                 predicate_value, payload_value);
  }
  node_to_ir_[node] = result;
  tokens_.push_back(result);
  return absl::OkStatus();
...
absl::Status FunctionConverter::HandleBuiltinRecv(const Invocation* node) {
  ...
  if (implicit_token_data_.has_value()) {
    ...
    value = builder_ptr->ReceiveIf(
        BReceiveChannelRef(channel_ref), token_value,
        implicit_token_data_->create_control_predicate());
  } else {
    value = builder_ptr->Receive(BReceiveChannelRef(channel_ref), token_value);
  }
...
absl::Status FunctionConverter::HandleBuiltinRecvNonBlocking(
  ...
  if (implicit_token_data_.has_value()) {
    ...
    recv = builder_ptr->ReceiveNonBlocking(BReceiveChannelRef(channel_ref),
                                           token_value);
  }
...
absl::Status FunctionConverter::HandleBuiltinRecvIf(const Invocation* node) {
  ...
  if (implicit_token_data_.has_value()) {
    ...
    recv = builder_ptr->ReceiveIf(BReceiveChannelRef(channel_ref), token_value,
                                  predicate_value);
  }
  BValue new_token_value = builder_ptr->TupleIndex(recv, 0);
  BValue received_value = builder_ptr->TupleIndex(recv, 1);
...
absl::Status FunctionConverter::HandleBuiltinRecvIfNonBlocking(
  ...
  if (implicit_token_data_.has_value()) {
    ...
    recv = builder_ptr->ReceiveIfNonBlocking(BReceiveChannelRef(channel_ref),
                                             token_value, predicate_value);
  }
  BValue new_token_value = builder_ptr->TupleIndex(recv, 0);
  BValue received_value = builder_ptr->TupleIndex(recv, 1);
  BValue receive_activated = builder_ptr->TupleIndex(recv, 2);
...
FunctionConverter::LowerFuzzTestDomains(Function* node) {
  FuzzTestConverter converter(current_type_info_, import_data_);
...
absl::Status FunctionConverter::HandleSpawn(const Spawn* node) {
  ...
  for (Expr* arg : invocation->args()) {
    ...
    if (std::holds_alternative<ChannelArray*>(arg_value)) {
      ...
      XLS_ASSIGN_OR_RETURN(
          ChannelInterface * channel_interface,
          IrValueToChannelInterface(arg_value, channel_type->direction()));
      channel_args.push_back(channel_interface);
    }
  }
...
absl::Status FunctionConverter::DefineProcDefChannelOrArrayIfLocal(
    const ProcDef* proc_def, const InterpValue& value, const TypeInfo* ti) {
  if (channel_or_array_id_to_object_.contains(GetChannelOrArrayId(value))) {
    return absl::OkStatus();
  }
  ...
  std::optional<const Function*> containing_fn = GetContainingFunction(definer);
  if (!containing_fn.has_value()) {
    return absl::OkStatus();
  }

  XLS_ASSIGN_OR_RETURN(std::optional<const ProcDef*> containing_proc,
                       GetProcConstructedByFunction(*containing_fn, ti));
  if (!containing_proc.has_value() || *containing_proc != proc_def) {
    return absl::OkStatus();
  }

  VLOG(5) << "Defining channel: " << value.ToString()
          << ", which is created locally in " << proc_def->identifier()
          << " and only used to construct sub-procs.";

  XLS_ASSIGN_OR_RETURN(const Type* channel_decl_type,
                       ti->GetItemOrError(definer));
  ChannelDirection direction = GetChannelOrArrayDirection(value);
  XLS_RET_CHECK(channel_decl_type->IsTuple());
  XLS_RET_CHECK_EQ(channel_decl_type->AsTuple().size(), 2);
  const Type& direction_type = channel_decl_type->AsTuple().GetMemberType(
      direction == ChannelDirection::kOut ? 0 : 1);
  absl::flat_hash_map<const ChannelDecl*, ChannelOrArray> channel_decls;
  return DefineProcDefChannelOrArray<ChannelDecl>(
      absl::down_cast<const ChannelDecl*>(definer),
...
absl::Status FunctionConverter::DefineProcDefChannelOrArray(
  ...
  if (definer->kind() == AstNodeKind::kChannelDecl) {
    ...
    if (name_def.has_value()) {
      SetNodeToChannelOrArray(*name_def, it->second);
    }
    return absl::OkStatus();
  }
  ...
  if (name_def.has_value()) {
    SetNodeToChannelOrArray(*name_def, channel_or_array);
  }
  VLOG(10) << "Populating channel for " << value.ToString() << " in conv "
           << std::hex << (uint64_t)this;
...
absl::Status FunctionConverter::InitProcDefChannels(
  ...
  proc_def_channel_scope_->EnterFunctionContext(current_type_info_,
                                                std::move(env));
  ...
  for (int i = 0; i < proc_type.members().size(); i++) {
    ...
    StructMemberNode* member = proc_def->members()[i];
    ...
    XLS_RETURN_IF_ERROR(DefineProcDefChannelOrArray<StructMemberNode>(
        member, member->name_def(), initializer, member_type, channel_decls));
  }
  ...
  for (const auto& [param, initializer] :
    ...
    if (!initializer.IsChannelReference() && !initializer.IsChannelArray()) {
      continue;
    }
    XLS_RETURN_IF_ERROR(DefineProcDefChannelOrArray<Param>(
        param, param->name_def(), initializer, param_type, channel_decls));
  }

  return absl::OkStatus();
}

absl::Status FunctionConverter::InitProcDefStateElements(
    const ProcDef* proc_def,
    const InterpValue::ProcInitializer& canonical_initializer) {
  ...
  for (int i = 0; i < state_elements.size(); i++) {
    StructMemberNode* state_member_node = state_elements[i];
    VLOG(5) << "Configuring state element: " << state_member_node->name();

    state_read_called_by_state_name_[state_member_node->name()] =
        function_builder_->Literal(UBits(0, 1));
    state_write_called_by_state_name_[state_member_node->name()] =
        function_builder_->Literal(UBits(0, 1));
    ...
    std::string state_name = absl::StrCat("__", state_member_node->name());
    const InterpValue& member_value =
        canonical_initializer.GetMemberValue(state_member_node);
...
absl::StatusOr<std::unique_ptr<FunctionConverter::ProcDefInstance>>
FunctionConverter::CreateProcDefInstance(const ProcDef* proc_def) {
  absl::flat_hash_map<std::string, IrValue> member_values;
  for (const StructMemberNode* node : proc_def->members()) {
    std::optional<IrValue> value = GetNodeToIr(node);
    XLS_RET_CHECK(value.has_value())
        << "Expected IR value for member `" << node->name() << "`";
    member_values[node->name()] = *value;
  }
...
absl::Status FunctionConverter::ExpandProcDefChannelReference(
  ...
  if (std::holds_alternative<ChannelArray*>(channel_or_array)) {
    // Get all the channel interfaces of this array and
    // add them to the channel_args
    ChannelArray* channel_array = std::get<ChannelArray*>(channel_or_array);
    for (ChannelRef channel_ref : channel_array->channels()) {
      XLS_ASSIGN_OR_RETURN(
          ChannelInterface * channel_interface,
          absl::visit(
              Visitor{[&](Channel* c) -> absl::StatusOr<ChannelInterface*> {
                        return ChannelToInterface(c, direction);
                      },
                      [&](ChannelInterface* ci)
                          -> absl::StatusOr<ChannelInterface*> { return ci; }},
              channel_ref));
    ...
    XLS_ASSIGN_OR_RETURN(
        ...
        IrValueToChannelInterface(
            absl::ConvertVariantTo<IrValue>(channel_or_array), direction));
    out.push_back(channel_interface);
  }
...
absl::Status FunctionConverter::AddProcDefInstantiation(
    ...
    const InterpValue& canonical_initializer, const TypeInfo* constructor_ti) {
  std::vector<ChannelInterface*> channel_args;
  ...
  for (int i = 0; i < external_members.size(); i++) {
    ...
    VLOG(10) << "Passing channel or array `" << external_arg.ToString()
             << "` in proc instantiation `" << canonical_initializer.ToString()
             << "`";
    XLS_RETURN_IF_ERROR(DefineProcDefChannelOrArrayIfLocal(
        proc_def, external_arg, constructor_ti));
    XLS_RETURN_IF_ERROR(
        ExpandProcDefChannelReference(external_arg, channel_args));
  }
  ...
  const auto proc_it =
      package_data_.canonical_proc_def_initializer_to_ir_proc.find(
          canonical_initializer);
  XLS_RET_CHECK(proc_it !=
                package_data_.canonical_proc_def_initializer_to_ir_proc.end())
      << "Expected to have an IR proc for `" << canonical_initializer.ToString()
      << "` which is spawned from `" << proc_def->identifier()
      << "` in instantiation `" << external_initializer.ToString() << "`";
  xls::Proc* spawned_proc = proc_it->second;

  xls::Proc* current_proc =
      absl::down_cast<ProcBuilder*>(function_builder_.get())->proc();
...
absl::Status FunctionConverter::ConvertProcDef(
  ...
  XLS_RETURN_IF_ERROR(
      InitProcDefChannels(proc_def, canonical_initializer, constructor_ti));
  XLS_RETURN_IF_ERROR(
  ...
  for (const InterpValue& spawnee : spawnees) {
    ...
    XLS_RETURN_IF_ERROR(AddProcDefInstantiation(
        proc_def, spawnee, canonical_initializer.initializer, constructor_ti));
  }
  ...
  // TODO: https://github.com/google/xls/issues/4125 - Augment this data
...
absl::Status FunctionConverter::HandleProcNextFunction(
  ...
  ParametricEnv bindings(parametric_env_map_);
  std::unique_ptr<ProcScopedChannelScope> proc_scoped_channel_scope;
  if (options_.lower_to_proc_scoped_channels) {
    ...
    proc_scoped_channel_scope->EnterFunctionContext(current_type_info_,
                                                    bindings);
    // Overwrite channel scope for the remains of this function call with
    // the proc-scoped version. Since function converters are thrown away after
    // each function is processed, this is fine.
...
absl::Status FunctionConverter::HandleSplatStructInstance(
  ...
  std::vector<BValue> members;
  for (int64_t i = 0; i < def->members().size(); ++i) {
    const std::string& k = def->GetMemberName(i);
...
absl::Status FunctionConverter::HandleAttr(const Attr* node) {
  ...
  // If it's a reference to `self.something` where `self` is a proc, we need
  // `IrValue`-based logic, because it may be a channel or other thing that a
...
FunctionConverter::DerefStructOrEnum(TypeDefinition node) {
  ...
  if (std::holds_alternative<ProcDef*>(node)) {
    return std::get<ProcDef*>(node);
  }
...
absl::StatusOr<StructDefBase*> FunctionConverter::DerefStructOrProc(
    TypeDefinition node) {
  XLS_ASSIGN_OR_RETURN(DerefVariant v, DerefStructOrEnum(node));
  XLS_RET_CHECK(std::holds_alternative<StructDef*>(v) ||
                std::holds_alternative<ProcDef*>(v));
  return absl::down_cast<StructDefBase*>(ToAstNode(v));
}
